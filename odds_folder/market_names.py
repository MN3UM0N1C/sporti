...
class FootballDataParser:
    ...
    def search(self, league_id, match, casino=None):
        ...
        try:
            ...
            for k in soup.find_all("match", {"id": match}):
                if casino is not None:
                    for bookmakers in k.find_all('type', {'value': True}):
                        if "Handicap" not in bookmakers["value"] and bookmakers.find("bookmaker")["name"] == casino:
                            result.append(bookmakers.find("bookmaker"))
                else:
                    for bookmakers in k.find_all('type', {'value': True}):
            ...
            for k in soup.find_all("match", {"id": match}):
                if casino is not None:
                    ...
                    for bookmakers in k.find_all('type'):
                        if "Handicap" not in bookmakers["value"]:
                            for one_casino in bookmakers.find_all("bookmaker"):
                                all_casino.append(one_casino)
                            result[bookmakers["value"]] = all_casino
            return result
        except AttributeError:
            print("Error: Data structure is not as expected. Check if the XML file has the correct structure.")
    ...
    def koef(self, file):
        ...
        for p, i in file.items():
            ...
            for k in i.find_all('odd'):
                data.append((k.get('name'), k.get('value')))
        return json.dumps(dict(data))
...
if __name__ == "__main__":
    mma_parser = BasketballDataParser()
    #mma_parser.download_all_files()
    print(mma_parser.koef(mma_parser.team("nba", ['Portland Trail Blazers', 'Detroit Pistons'], casino="Marathon")))
