...
class FootballerScraper:
    def __init__(self, user_input):
        ...
        #     "DNT": "1",
        #     "X-Forwarded-For": "127.1111.0.1",
        #     "Cookie": "asdasj ejjeaaaas wd de"
        # }
    ...
    def names(self, teams=False, not_all=False):
        #print(requests.get("http://api.ipify.org", headers=self.headers, proxies=self.proxy_to_use).text)
        global teams_page
        info = BeautifulSoup(self.content.text, 'html.parser')
        if "Why Not Provide an API?" in self.content.text:
            print("BLOCKED")
        names = []
